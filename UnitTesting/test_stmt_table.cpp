...
TEST_CLASS(TestStmtTable) {
  ...
  const int kSampleStmtIdx1 = 1;
  const int kSampleStmtIdx2 = 2;
  ...
  TEST_METHOD(TestGetAllStmtNumsSingle) {
    StmtTable stmt_table;
    stmt_table.InsertStmt(kSampleStmtIdx1, kSampleStmt1);
    int* stmt_nums = stmt_table.GetAllStmtNums();
    Assert::AreEqual(kSampleStmtIdx1, stmt_nums[0]);
  }
  ...
  TEST_METHOD(TestGetAllStmtNumsMultiple) {
    ...
    int* stmt_nums = stmt_table.GetAllStmtNums();
    Assert::AreEqual(kSampleStmtIdx1, stmt_nums[0]);
    Assert::AreEqual(kSampleStmtIdx2, stmt_nums[1]);
  }
};
}