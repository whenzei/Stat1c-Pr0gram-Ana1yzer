...
void PqlQuery::AddSelection(string selection, PqlDeclarationEntity type) {
  ...
  if (selection[0] == '0')
    optimizer_.AddSelection(selection.substr(1));
  else
...
void PqlQuery::AddClause(PqlClause* clause) {
  ...
  if (clause->GetClauseType() == PqlClauseType::kPattern) {
    ...
    if (pattern->GetType().second == PqlPatternType::kAssign)
      type = PqlDeclarationEntity::kAssign;
    else if (pattern->GetType().second == PqlPatternType::kWhile)
      type = PqlDeclarationEntity::kWhile;
    else if (pattern->GetType().second == PqlPatternType::kIf)
      type = PqlDeclarationEntity::kIf;
    optimizer_.AddUnion(clause, pattern->GetType().first, type,
                        pattern->GetFirstParameter().first,
                        pattern->GetFirstParameter().second);
    ...
    optimizer_.AddUnion(clause, param1, param.first.second, param2,
                        param.second.second);
