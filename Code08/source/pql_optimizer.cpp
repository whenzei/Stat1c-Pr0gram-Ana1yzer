...
vector<PqlGroup> PqlOptimizer::Optimize() {
  ...
  for (unsigned i = 0; i < clauses_.size(); i++) {
    if (clauses_[i]->GetSynonyms().first != "") {  // if there is a synonym
      ...
      if (group_map.find(root) == group_map.end()) {  // if group don't exist
        ...
        if (ClauseUsesSelection(clauses_[i]))
          group_map.at(root).UsesSelectionTrue();
      } else {
        group_map.at(root).AddClause(clauses_[i]);
        if (ClauseUsesSelection(clauses_[i]))
          group_map.at(root).UsesSelectionTrue();
      ...
      if (group_map.find(-1) == group_map.end()) {  // if group don't exist
        ...
        if (ClauseUsesSelection(clauses_[i]))
          group_map.at(-1).UsesSelectionTrue();
      } else {
        group_map.at(-1).AddClause(clauses_[i]);
        if (ClauseUsesSelection(clauses_[i]))
          group_map.at(-1).UsesSelectionTrue();
  ...
  for (unordered_map<int, PqlGroup>::iterator it = group_map.begin();
       it != group_map.end(); it++) {
    PqlGroup group = it->second;
    group.SortClauses();  // sort clauses within a group
    if (it->first == -1)
      group.SetPriority(
          INT_MAX);  // set groups with no synonyms to max priority
    else if (!group.GetUsesSelection())
      group.SetPriority(INT_MAX - 1);  // set groups that do not use selected
                                       // synonyms as 2nd top priority
...
void PqlOptimizer::AddUnion(PqlClause* clause, string first,
                            PqlDeclarationEntity first_type, string second,
                            PqlDeclarationEntity second_type) {
  clauses_.push_back(clause);

  // CASE 1: Both are synoynms
  if (PqlValidator::IsSynonym(first_type) &&
      PqlValidator::IsSynonym(second_type)) {
    ...
    // CASE 1b: Both have groups
      ...
      if (first_group != second_group) {
        ...
        // CASE 1c.2: Only 1 have group ref
          ...
          if (find_.at(first_group) == -1)
            find_[second_group] = first_group;
          else
            find_[first_group] = second_group;
        }
      }
    }
    // CASE 1c: 1 of them has group
      ...
      if (first_group != -1)
        union_.insert({second, first_group});
      else
        union_.insert({first, second_group});
    }
  }
  // CASE 2: Only 1 synonym
  else if (PqlValidator::IsSynonym(first_type) ||
           PqlValidator::IsSynonym(second_type)) {
    string synonym;
    if (PqlValidator::IsSynonym(first_type))
      synonym = first;
...
bool PqlOptimizer::ClauseUsesSelection(PqlClause* clause) {
  return (selections_.find(clause->GetSynonyms().first) != selections_.end() ||
          selections_.find(clause->GetSynonyms().second) != selections_.end());
}
