...
TEST_CLASS(TestValidator) {
 public:
  ...
  TEST_METHOD(TestIsValidStatement) {
    ...
    // passes correct while statement (will check fail cases in
    // TestIsValidWhileBlock)
  ...
  TEST_METHOD(TestIsValidIfBlock) {
    ...
    TokenList tokens =
        Tokenizer::Tokenize("(a > b) then { a = b;} else { c = d;}");
    Validator validator = Validator(tokens);
    Assert::IsTrue(validator.IsValidIfBlock());

    // passes if multiple statements
    tokens = Tokenizer::Tokenize(
        "(a > b) then {a = b; b = c;} else {c = d; d = e;}");
    validator.ResetTokenList(tokens);
    Assert::IsTrue(validator.IsValidIfBlock());

    // passes even with nested while block
    tokens = Tokenizer::Tokenize(
        "(a > b) then {a = b; while(d > e) { d=e;}} else {c = d;}");
    validator.ResetTokenList(tokens);
    Assert::IsTrue(validator.IsValidIfBlock());

    tokens = Tokenizer::Tokenize(
        "(a > b) then {while(d > e) { d=e;} a = b; } else {c = d;}");
    validator.ResetTokenList(tokens);
    Assert::IsTrue(validator.IsValidIfBlock());

    // passes even with nested ifelse block
    tokens = Tokenizer::Tokenize(
        "(a > b) then {a = b;} else {c = d; if(d > e) then {d = e;} else { "
        "e=f;}}");
    validator.ResetTokenList(tokens);
    Assert::IsTrue(validator.IsValidIfBlock());
    ...
    tokens =
        Tokenizer::Tokenize("(a > b) then { 234 = a + b; } else { a = b;}");
    validator.ResetTokenList(tokens);
    Assert::IsFalse(validator.IsValidIfBlock());
  ...
  TEST_METHOD(TestIsValidWhileBlock) {
    ...
    tokens = Tokenizer::Tokenize(
        "(a > b) {a = b; if(c > d) then { c = d;} else {d = c;}}");
    validator.ResetTokenList(tokens);
    Assert::IsTrue(validator.IsValidWhileBlock());

    tokens = Tokenizer::Tokenize(
        "(a > b) {if(c > d) then { c = d;} else {d = c;} a = b; }");
    validator.ResetTokenList(tokens);
    Assert::IsTrue(validator.IsValidWhileBlock());

    tokens =
        Tokenizer::Tokenize("(a > b) {if(c > d) then { c = d;} else {d = c;}}");
    validator.ResetTokenList(tokens);
    Assert::IsTrue(validator.IsValidWhileBlock());
  ...
  // directly it will generate a kEOF token at the end which will cause it to
  // fail, thus remove the final token before passing it into the function
  TEST_METHOD(TestIsValidConditional) {
    ...
    // fails cond_expr if lhs and rhs of a statement with a conditional is
    // invalid
    tokens = Tokenizer::Tokenize("(a) || (b)");
    tokens.pop_back();
    Assert::IsFalse(validator.IsValidConditional(tokens));
