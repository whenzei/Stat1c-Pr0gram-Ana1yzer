...
TEST_CLASS(TestTokenizer){
  public :
	...
	TEST_METHOD(TestTokenizeCharacter) {
		...
		Result actual_result = Tokenizer::TokenizeCharacter(kTestType, 'h', kTestInput, 0);
	...
	TEST_METHOD(TestTokenizePattern) {
		...
		// check tokenize names (basically same call as TokenizeNames(...) starting at index 0
		Result expected_result({ 5, {kTestType, "hello"} });
		Result actual_result = Tokenizer::TokenizePattern(kTestType, regex{ R"([a-zA-Z][a-zA-Z0-9]*)" }, kTestInput, 0);
		Assert::IsTrue(actual_result == expected_result);

		// check tokenize names starting at index 2
		expected_result = Result({ 3, {kTestType, "llo"} });
		actual_result = Tokenizer::TokenizePattern(kTestType, regex{ R"([a-zA-Z][a-zA-Z0-9]*)" }, kTestInput, 2);
		Assert::IsTrue(actual_result == expected_result);

		// check no tokenizing of digits (using digit regex)
		actual_result = Tokenizer::TokenizePattern(kTestType, regex{ R"([0-9]+)" }, kTestInput, 2);
		Assert::IsFalse(actual_result == expected_result);
		Assert::IsTrue(actual_result == Tokenizer::EmptyResult());
	}
	...
	TEST_METHOD(TestSkipComments) {
		...
		// if input is a comment, should return result with correct numbers of consumed characters
	...
	TEST_METHOD(TestTokenizeBraces) {
		...
		actual_result = Tokenizer::TokenizeBraces(kTestInput, 2);
		Assert::IsTrue(actual_result == Tokenizer::EmptyResult());
	}
	...
	TEST_METHOD(TestTokenize) {
		string kTestInput("procedure one {\na= b;\n c  = d; \\comments here\n }");
		TokenList actual_tokens = Tokenizer::Tokenize(kTestInput);
		Assert::IsTrue(actual_tokens.size() == 12);
		TokenList expected_tokens({ {2, "procedure"}, {2, "one"}, {3, "{"}, {2, "a"}, {5, "="},
			{2, "b"}, {4, ";"}, {2, "c"}, {5, "="}, {2, "d"}, {4, ";"}, {3, "}"} });
