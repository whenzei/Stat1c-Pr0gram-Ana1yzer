...
bool PqlQuery::AddClause(PqlClause* clause) { 
  ...
  if (clause->GetClauseType() == PqlClauseType::kPattern) {
    ...
    return optimizer_.AddUnion(clause, pattern->GetType().first, type, pattern->GetFirstParameter().first, pattern->GetFirstParameter().second);
    ...
    return optimizer_.AddUnion(clause, param1, param.first.second, param2, param.second.second);
